...
app_path := if host_os == "windows" { build_dir / build_type / "AmpuhGen1.exe" } else if host_os == "macos" { build_dir / build_type / "AmpuhGen1.app" / "Contents" / "MacOS" / "AmpuhGen1" } else { build_dir / build_type / "AmpuhGen1" }
...
submodules:
    @{{ python }} -c "import os, subprocess, shutil; subprocess.run(['git', 'submodule', 'update', '--init', '--recursive'], check=False) if (os.path.exists('.git') and shutil.which('git')) else None"
...
# Build the project
build:
    {{ python }} ./tools/build.py --preset {{ build_preset }} --parallel {{ jobs }}

# Configure and build Release
release:
    {{ python }} ./tools/configure.py --preset default-release -B {{ build_dir }} --release {{ qt_root_arg }}
    {{ python }} ./tools/build.py --preset default-release --parallel {{ jobs }}
