...
def find_repo_root(start_path: Path | None = None) -> Path:
    """
    Find the repository root by looking for .git directory, CMakeLists.txt, or tools/pyproject.toml.

    Args:
        start_path: Starting point for search. Defaults to current file.

    Returns:
        Path to repository root, or directory containing start_path if not found.
    """
    ...
    for parent in [current, *current.parents]:
        if (parent / ".git").exists():
            return parent
        if (parent / "CMakeLists.txt").exists() and (parent / "tools" / "pyproject.toml").exists():
            return parent
